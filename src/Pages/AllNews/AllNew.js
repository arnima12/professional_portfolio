...
const AllNew = ({ latestNew, allNew, email }) => {
    const news = latestNew || allNew;
    const { title, image, date, id, desc } = news;
    console.log("news", title)
    if (latestNew) {
        return (
            <div className="latestBlog flex flex-col-reverse lg:flex-row gap-8 mb-8 w-[93%] py-8">
                <div className="w-[16rem] md:w-[40rem] text-left pb-8 ml-12">
                    <h2 className="font-[500] text-[30px] md:text-[48px] text-[rgb(47,94,163)]">{title}</h2>
                    <h3 className="text-[rgb(42,193,235)] font-[800] mb-4">{date}</h3>
                    <p className="font-[600]">{desc?.slice(0, 600)}</p>
                    <div>
                        <Link to={`/${email}/news/${id}`} state={{ news }}>
